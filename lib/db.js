...
module.exports.getClassNameByUserName = function(user_name,callback){
	connection.query('SELECT classes.class_name FROM classes,players WHERE players.user_name = ? AND players.class_id = classes.id',[user_name], function(err, rows, fields) {
		if (err){
			throw err;
		}
...
module.exports.setClassForUser = function(user_name,class_name,callback){
	this.getClassIdByName(class_name,function(data){
		console.log(data);

		connection.query('UPDATE players SET class_id = ? WHERE players.user_name = ?',[data,user_name], function(err, rows, fields) {
			if (err){
				throw err;
			}
...
module.exports.getTopUserByClass = function(className,callback){
	this.getClassIdByName(className,function(class_id){
		console.log("class_id ===============",class_id);
		connection.query('SELECT * FROM players WHERE class_id = ? ORDER BY gold DESC LIMIT 10', [class_id], function(err, rows, fields) {
			if (err){
				throw err;
			} 
			callback(rows);
