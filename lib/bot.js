...
function getRpg(req,res){
	...
	var botPayload = {
		text : "Bonjour, ' + getUser(req) + ', l'aide est disponible avec 'rpg help'."
	};
...
function getTop(req,res){
	db.getTopUserByGold(function(list){
		var text = "Le top "+ list.length +" des personnes les plus riches est : \n";
...
function getTopByClass(req,res,className){
	...
	db.getTopUserByClassName(className,function(list){
		var text = "Le top "+ list.length +" des "+className+" les plus riches est : \n";
		console.log(text);
		...
		if(!classUser){
			...
			db.subGold(getUser(req),cost,function(isOk,err){
				...
				if(isOk){
					...
					msg += getUser(req) + " a changer de class pour ["+ class_name+"]";
					console.log("msg = ",msg);
	...
	db.addClass(class_name,description,function(result,err){
		...
		if(err){
			if(err.code === "ER_DUP_ENTRY"){
				res.status(200).json({"text":"La classe "+class_name+" existe déjà."});
			}
		}else{
			res.status(200).json({"text":"La classe "+class_name+" a bien été ajoutée."});
		}
	});
}
...
function isAdmin(req,res,callback){
	db.isAdmin(getUser(req),function(isAdmin){
		if(isAdmin){
			...
			res.status(200).json({"text":"Vous n'avez pas les permissions nécessaire pour faire cela."});
			callback(false);
		}
	});
}
...
function giveGold(req,res,from,to,amount){
	if(amount < 0){
		res.status(200).json({"text":"Vous ne pouvez pas donner un nombre négatif de gold."});
		return;
	}
	if(amount === 0){
		res.status(200).json({"text":"Vous ne pouvez pas donner 0 gold."});
		return;
	}
	db.getGoldByUserName(getUser(req),function(gold){
		if(amount < gold){
			db.addGold(to,amount,function(){
				db.subGold(from,amount,function(){
					res.status(200).json({"text":"Vous avez donné "+amount+" à "+to});
				});
			});
		}else{
