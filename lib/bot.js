...
function getRpg(message){
	...
	var botPayload = {
		text : 'Hello ' + getUserName(message) + ', help available with "rpg help"'
	};
	sendMsg(botPayload.text, message.channel);
}
...
function getTop(message){
	db.getTopUserByGold(function(list){
		var msg = "Le top "+ list.length +" des personnes les plus riches sont : \n";
...
function getTopByClass(message,className){
	...
	db.getTopUserByClassName(className,function(list){
		var msg = "Le top "+ list.length +" des "+className+" les plus riches sont : \n";
		...
		if(!classUser){
			...
			db.subGold(getUserName(message),cost,function(isOk,err){
				...
				if(isOk){
					...
					msg += getUserName(message) + " à changer de class pour ["+ class_name+"]";
					console.log("msg = ",msg);
	...
	db.addClass(class_name,description,function(result,err){
		...
		if(err){
			if(err.code === "ER_DUP_ENTRY"){
				sendMsg("Class "+class_name+" already exist",message.channel);
			}
		}else{
			sendMsg("Class "+class_name+" successfully added",message.channel);
		}
	});
}
...
function isAdmin(message,callback){
	db.isAdmin(getUserName(message),function(isAdmin){
		if(isAdmin){
			...
			sendMsg("You don't have permissions to do that",message.channel);
			callback(false);
		}
	});
}
...
function giveGold(message,from,to,amount){
	if(amount < 0){
		sendMsg("Vous ne pouvez pas donnée un nombre negatif de gold",message.channel);
		return;
	}
	if(amount == 0){
		sendMsg("Vous ne pouvez pas donnée 0 gold",message.channel);
		return;
	}
	db.getGoldByUserName(getUserName(message),function(gold){
		if(amount < gold){
			db.addGold(to,amount,function(){
				db.subGold(from,amount,function(){
					sendMsg("Vous avez donnée "+amount+" a "+to,message.channel);
				});
			});
		}else{
...
function playGame(message){
	...
	if(gn){
		try{
			var game = require("../games/"+gn)(games,cmd);
			games[gn].router(games,cmd);
		}catch(e){
