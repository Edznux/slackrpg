			...
			db.addGold(to,amount,function(isOk){
				if(isOk){
					db.subGold(from,amount,function(isOk){
						if(isOk){
							sendMsg("Vous avez donné "+amount+" à "+ slack.getUserByID(to).name, message.channel);
						}else{
...
function giveGold(message,to,amount){
	...
	db.addGold(to,amount,function(isOk){
		if(isOk){
			sendMsg("[ADMIN] Vous avez donné "+amount+" à "+ slack.getUserByID(to).name, message.channel);
		}else{
			sendMsg("Erreur lors de la transaction");
		}
	});
